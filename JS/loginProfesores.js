document.getElementById('form-login-profe').addEventListener('submit', async function(event) {
    ...
    try {
        const respuesta = await fetch('./profesores.json'); // Minúsculas definitivas
        ...
        if (profesorEncontrado && profesorEncontrado.clave === passProfe) {
            localStorage.setItem('sesion_activa', JSON.stringify({ nombre: profesorEncontrado.nombre, rol: 'profe' }));
